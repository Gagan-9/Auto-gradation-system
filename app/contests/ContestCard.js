...
const ContestCard = ({ contestId, contestUrl, isUpcoming=false, isOngoing }) => {
  return (
    <Link href={`/contests/${contestUrl}`} className='w-full flex justify-between items-center bg-light-2 rounded-l-full rounded-r-full group pl-4 pr-10 py-2 hover:bg-green-700 transition-all ease-in max-lg:rounded-l-md max-sm:rounded-xl max-xs:flex-col max-xs:items-start hover:scale-[1.03] max-sm:pl-6'>
